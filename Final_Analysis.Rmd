...
```{r cellabund-vs-fraction, eval = TRUE, fig.width = 6, fig.height = 3.5, include = TRUE}
...
```{r pcoa, fig.width=8, fig.height=4, dependson = "load-data"}
...
```{r environ-lm, fig.height=3.5, fig.height=8, dependson = "euclidian-pcoa-varexplained"}
...
#pander(pca_lm_results_df_PC3,
#               caption = "Single Linear regression statistics for PC3.")
...
#pander(pca_lm_results_df_PC4,
#               caption = "Single Linear regression statistics for PC4.")
...
#pander(pca_lm_results_df_PC5,
#               caption = "Single Linear regression statistics for PC5.")
...
#pander(pca_lm_results_df_PC6,
#               caption = "Single Linear regression statistics for PC6.")
...
pander(all_pca_lm_results,
       caption = "All individual LM results from PCA axes.")

pander(dplyr::filter(all_pca_lm_results, p.value < 0.06),
...
poster_a <- ggplot(filter(metadata, norep_filter_name != "MOTEJ515"), 
  ...
  ##### Particle vs free cell abundances 
  geom_path(data = dat1, aes(x = a, y = b), linetype = 1, color = "gray40") +
  annotate("text", x=1.5, y=6.5, size = 4, color = "gray40",
           label= paste("***\np =", round(frac_abund_wilcox$p.value, digits = 6))) +
  theme(legend.position = "bottom",
        legend.title = element_blank(),
        axis.title.x = element_blank())
...
poster_b <- ggplot(metadata, aes(y = frac_bacprod, x = fraction)) + 
  ...
  annotate("text", x=1.5, y=72, size = 4, color = "gray40",
...
poster_c <- ggplot(filter(metadata, norep_filter_name != "MOTEJ515"), 
  ...
  annotate("text", x=1.5, y=-5,  size = 4, color = "gray40",
...
shannon_distribution_plot <- 
  ...
  annotate("text", x=1.5, y=5.4, size = 4, color = "#424645",
...
################ Figure S7: Per-cell Heterotrophic Production vs All Observed Richness
## 1. Extract the R2 and p-value from the linear model: 
...
## 2.  Figure S7: Per-cell Heterotrophic Production vs All Observed Richness
plot_all_rich_percell <- 
  ...
  # Add the R2 and p-value to the plot 
  annotate("text", x=790, y=-8, label=lm_lab_perliter_invsimps_PA, parse = TRUE, color = "#424645", size = 4) +
  theme(legend.title = element_blank(), legend.position ="bottom", 
        legend.text = element_text(size = 14))


################# CORRECT ME HERE MARIAN 
