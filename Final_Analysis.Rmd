...
relationships in freshwater bacterial communities"
...
library(broom)      # pvalues out of multiple regression
...
## Total Production 
```{r total-prod-vs-season-station, eval = TRUE, fig.width = 6, fig.height = 5, include = TRUE}
...
prod_by_station <- ggplot(dplyr::filter(metadata, year == "2015" & fraction %in% c("Free")), 
  ...
  xlab("Lake Station") + ylab("Total Production (ug C/cell/day)") + 
...
```{r euclidian-pcoa-varexplained, fig.height=4, fig.width=5, dependson = "environmental-pairwise"}
...
The first two axes of the PCA explain `r summary(pca_environ)$cont$importance[3, 2]*100`% of the variation in the environmental data!


# Linear Models with Environmental Variables
```{r environ-lm, fig.height=3.5, fig.width=8, dependson = "euclidian-pcoa-varexplained"}
...
#####  SINGLE REGRESSION with PC3
## Free living
comm_lm_PC3_free <- summary(lm(frac_bacprod ~ PC3, 
           data = filter(metadata_pca, fraction == "Free")))
## Particle-associated
comm_lm_PC3_part <- summary(lm(frac_bacprod ~ PC3, 
           data = filter(metadata_pca, fraction == "Particle")))
## PER CAPITA: Free living
percap_lm_PC3_free <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC3, 
           data = filter(metadata_pca, fraction == "Free")))
## PER CAPITA: Particle-associated
percap_lm_PC3_part <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC3, 
           data = filter(metadata_pca, fraction == "Particle")))

# Put the results all together 
pca_lm_row1_PC3 <- c("Free", "Per-Liter","PC3",
          round(comm_lm_PC3_free$adj.r.squared, digits = 3),
          round(comm_lm_PC3_free$coefficients[2,4], digits = 3))
pca_lm_row2_PC3 <- c("Particle", "Per-Liter","PC3",
          round(comm_lm_PC3_part$adj.r.squared, digits = 3),
          round(comm_lm_PC3_part$coefficients[2,4], digits = 3))
pca_lm_row3_PC3 <- c("Free", "Per-Capita","PC3",
          round(percap_lm_PC3_free$adj.r.squared, digits = 3),
          round(percap_lm_PC3_free$coefficients[2,4], digits = 3))
pca_lm_row4_PC3 <- c("Particle", "Per-Capita", "PC3",
          round(percap_lm_PC3_part$adj.r.squared, digits = 3),
          round(percap_lm_PC3_part$coefficients[2,4], digits = 3))

pca_lm_results_df_PC3 <- 
  rbind(pca_lm_row1_PC3, pca_lm_row2_PC3, pca_lm_row3_PC3, pca_lm_row4_PC3)

colnames(pca_lm_results_df_PC3) <- c("fraction", "Predictor","Prod_measure", "Adjusted_R2", "p-value")

row.names(pca_lm_results_df_PC3) = NULL

#####  SINGLE REGRESSION with PC4
## Free living
comm_lm_PC4_free <- summary(lm(frac_bacprod ~ PC4, 
           data = filter(metadata_pca, fraction == "Free")))
## Particle-associated
comm_lm_PC4_part <- summary(lm(frac_bacprod ~ PC4, 
           data = filter(metadata_pca, fraction == "Particle")))
## PER CAPITA: Free living
percap_lm_PC4_free <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC4, 
           data = filter(metadata_pca, fraction == "Free")))
## PER CAPITA: Particle-associated
percap_lm_PC4_part <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC4, 
           data = filter(metadata_pca, fraction == "Particle")))

# Put the results all together 
pca_lm_row1_PC4 <- c("Free", "Per-Liter","PC4",
          round(comm_lm_PC4_free$adj.r.squared, digits = 3),
          round(comm_lm_PC4_free$coefficients[2,4], digits = 3))
pca_lm_row2_PC4 <- c("Particle", "Per-Liter","PC4",
          round(comm_lm_PC4_part$adj.r.squared, digits = 3),
          round(comm_lm_PC4_part$coefficients[2,4], digits = 3))
pca_lm_row3_PC4 <- c("Free", "Per-Capita","PC4",
          round(percap_lm_PC4_free$adj.r.squared, digits = 3),
          round(percap_lm_PC4_free$coefficients[2,4], digits = 3))
pca_lm_row4_PC4 <- c("Particle", "Per-Capita", "PC4",
          round(percap_lm_PC4_part$adj.r.squared, digits = 3),
          round(percap_lm_PC4_part$coefficients[2,4], digits = 3))

pca_lm_results_df_PC4 <- 
  rbind(pca_lm_row1_PC4, pca_lm_row2_PC4, pca_lm_row3_PC4, pca_lm_row4_PC4)

colnames(pca_lm_results_df_PC4) <- c("fraction", "Predictor","Prod_measure", "Adjusted_R2", "p-value")

row.names(pca_lm_results_df_PC4) = NULL


#####  SINGLE REGRESSION with PC5
## Free living
comm_lm_PC5_free <- summary(lm(frac_bacprod ~ PC5, 
           data = filter(metadata_pca, fraction == "Free")))
## Particle-associated
comm_lm_PC5_part <- summary(lm(frac_bacprod ~ PC5, 
           data = filter(metadata_pca, fraction == "Particle")))
## PER CAPITA: Free living
percap_lm_PC5_free <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC5, 
           data = filter(metadata_pca, fraction == "Free")))
## PER CAPITA: Particle-associated
percap_lm_PC5_part <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC5, 
           data = filter(metadata_pca, fraction == "Particle")))

# Put the results all together 
pca_lm_row1_PC5 <- c("Free", "Per-Liter","PC5",
          round(comm_lm_PC5_free$adj.r.squared, digits = 3),
          round(comm_lm_PC5_free$coefficients[2,4], digits = 3))
pca_lm_row2_PC5 <- c("Particle", "Per-Liter","PC5",
          round(comm_lm_PC5_part$adj.r.squared, digits = 3),
          round(comm_lm_PC5_part$coefficients[2,4], digits = 3))
pca_lm_row3_PC5 <- c("Free", "Per-Capita","PC5",
          round(percap_lm_PC5_free$adj.r.squared, digits = 3),
          round(percap_lm_PC5_free$coefficients[2,4], digits = 3))
pca_lm_row4_PC5 <- c("Particle", "Per-Capita", "PC5",
          round(percap_lm_PC5_part$adj.r.squared, digits = 3),
          round(percap_lm_PC5_part$coefficients[2,4], digits = 3))

pca_lm_results_df_PC5 <- 
  rbind(pca_lm_row1_PC5, pca_lm_row2_PC5, pca_lm_row3_PC5, pca_lm_row4_PC5)

colnames(pca_lm_results_df_PC5) <- c("fraction", "Predictor","Prod_measure", "Adjusted_R2", "p-value")

row.names(pca_lm_results_df_PC5) = NULL


#####  SINGLE REGRESSION with PC6
## Free living
comm_lm_PC6_free <- summary(lm(frac_bacprod ~ PC6, 
           data = filter(metadata_pca, fraction == "Free")))
## Particle-associated
comm_lm_PC6_part <- summary(lm(frac_bacprod ~ PC6, 
           data = filter(metadata_pca, fraction == "Particle")))
## PER CAPITA: Free living
percap_lm_PC6_free <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC6, 
           data = filter(metadata_pca, fraction == "Free")))
## PER CAPITA: Particle-associated
percap_lm_PC6_part <- summary(lm(log10(fracprod_per_cell_noinf) ~ PC6, 
           data = filter(metadata_pca, fraction == "Particle")))

# Put the results all together 
pca_lm_row1_PC6 <- c("Free", "Per-Liter","PC6",
          round(comm_lm_PC6_free$adj.r.squared, digits = 3),
          round(comm_lm_PC6_free$coefficients[2,4], digits = 3))
pca_lm_row2_PC6 <- c("Particle", "Per-Liter","PC6",
          round(comm_lm_PC6_part$adj.r.squared, digits = 3),
          round(comm_lm_PC6_part$coefficients[2,4], digits = 3))
pca_lm_row3_PC6 <- c("Free", "Per-Capita","PC6",
          round(percap_lm_PC6_free$adj.r.squared, digits = 3),
          round(percap_lm_PC6_free$coefficients[2,4], digits = 3))
pca_lm_row4_PC6 <- c("Particle", "Per-Capita", "PC6",
          round(percap_lm_PC6_part$adj.r.squared, digits = 3),
          round(percap_lm_PC6_part$coefficients[2,4], digits = 3))

pca_lm_results_df_PC6 <- 
  rbind(pca_lm_row1_PC6, pca_lm_row2_PC6, pca_lm_row3_PC6, pca_lm_row4_PC6)

colnames(pca_lm_results_df_PC6) <- c("fraction", "Predictor","Prod_measure", "Adjusted_R2", "p-value")

row.names(pca_lm_results_df_PC6) = NULL


################ Per-Capita production correlation with PC2
## 1. Run the linear regression 
lm_PC2_percap_part <- lm(log10(fracprod_per_cell_noinf) ~ PC2, data = dplyr::filter(metadata_pca, fraction == "Particle"))

# Linear models with PC1 and free-living PC2 are NOT SIGNIFICANT
summary(lm(log10(fracprod_per_cell_noinf) ~ PC2, data = dplyr::filter(metadata_pca, fraction == "Free")))
summary(lm(log10(fracprod_per_cell_noinf) ~ PC1, data = dplyr::filter(metadata_pca, fraction == "Free")))
summary(lm(log10(fracprod_per_cell_noinf) ~ PC1, data = dplyr::filter(metadata_pca, fraction == "Particle")))

# 2. Extract the R2 and p-value from the linear model: 
lm_lab_PC2_percap_part <- paste("atop(R^2 ==", round(summary(lm_PC2_percap_part)$adj.r.squared, digits = 3), ",",
            "p ==", round(unname(summary(lm_PC2_percap_part)$coefficients[,4][2]), digits = 3), ")")
...
PC1_lm_plot <- 
  ...
  geom_point() + ylab("log10(Per-Capita Production)") + 
...
PC2_lm_plot <- 
  ...
  geom_point(aes(color = fraction,  fill = fraction)) + ylab("log10(Per-Capita Production)") + 
  scale_color_manual(values = fraction_colors) + 
  scale_fill_manual(values = fraction_colors) + 
  ...
  annotate("text", y = -5.8, x=0.5, label=lm_lab_PC2_percap_part, parse = TRUE, color = "#FF6600", size = 4) +
  theme(legend.title = element_blank(), legend.position = "bottom")  

plot_grid(PC1_lm_plot, PC2_lm_plot, 
          labels = c("A", "B"),
          nrow = 1, ncol = 2)
```
...
**Important!** In a multiple regression, PC1 and PC2 correlate with log~10~(per-capita heterotrophic production)! With an adjusted R^2^ of `r round(summary(lm_percap_PC12_part)$adj.r.squared, digits = 3)` and a p-value of  `r round(glance(lm_percap_PC12_part)$p.value, digits = 4)`



## Filtered PCA Linear Model Results

Below represents the significant LM results from the individual PCA axes: 

```{r table-with-PC-results, warnings = FALSE}
# Put all of the PCA and environmental  linear model results together into one dataframe 
all_pca_lm_results <- dplyr::bind_rows(
  data.frame(pca_lm_results_df), 
  data.frame(pca_lm2_results_df), 
  data.frame(pca_lm_results_df_PC3),
  data.frame(pca_lm_results_df_PC4), 
  data.frame(pca_lm_results_df_PC5), 
  data.frame(pca_lm_results_df_PC6))

# Output the table in fancy form 
datatable(all_pca_lm_results, options = list(pageLength = 10)) 

# Output the results for models that have a pvalue of less than 0.05
pander(dplyr::filter(all_pca_lm_results, p.value < 0.05),
       caption = "Individual Linear Model results from the individual PCA axes with a p-value less than 0.06.")
```


```{r multiple-regression-PCs}
# Multiple regression with community-wide production
summary(lm(frac_bacprod ~ PC1 + PC2, data = filter(metadata_pca, fraction == "Particle")))

summary(lm(Temp_C ~ PC2, data = filter(metadata_pca, fraction == "Particle")))
summary(lm(pH ~ PC2, data = filter(metadata_pca, fraction == "Particle")))


ggplot(metadata_pca, aes(PC2, Temp_C)) + 
  geom_point() +
  geom_smooth(method = "lm")

ggplot(metadata_pca, aes(PC2, pH)) + 
  geom_point() +
  geom_smooth(method = "lm")

# Multiple regression with per-capita production
summary(lm(log10(fracprod_per_cell_noinf) ~ PC1 + PC2, data = filter(metadata_pca, fraction == "Free")))
```


# Figure S10: PCA with Euclidian Distances 
```{r euclidian-pcoa, fig.height=6, fig.width=6, dependson = c("environmental-pairwise", "euclidian-pcoa-varexplained")}
...
# Environmental Params: as they are

```{r basic-env-regressions}
metadata_pca_particle <- metadata_pca %>%
  filter(fraction == "Particle")

# Temperature
summary(lm(frac_bacprod ~ Temp_C, data = metadata_pca_particle))
summary(lm(log10(fracprod_per_cell_noinf) ~ Temp_C, data = metadata_pca_particle))

summary(lm(log10(fracprod_per_cell_noinf) ~ Temp_C, data = metadata_pca_particle))

# pH
summary(lm(frac_bacprod ~ pH, data = metadata_pca_particle))
summary(lm(log10(fracprod_per_cell_noinf) ~ pH, data = metadata_pca_particle))


# SO4_mgperL
summary(lm(frac_bacprod ~ SO4_mgperL, data = metadata_pca_particle))
summary(lm(log10(fracprod_per_cell_noinf) ~ SO4_mgperL, data = metadata_pca_particle))
```



## Figure 1
```{r boxplot-cellcount-prod, eval = TRUE, fig.width = 8.5, fig.height = 3.5, warning = FALSE, dependson="load-data"}
######################################################### Fraction ABUNDANCe 
...
#fig_1 <- 
  plot_grid(row1_plots, get_legend(poster_a),
                   ncol = 1, nrow = 2, 
...
```{r calc-div, dependson="environ-lm"}
...
```{r subset-div-data, dependson= "calc-div"}
#################################### Subset Dataframes 
...
### R2 table 
```{r r2-table, dependson = "div-prod-lm-models"}
...
## Prepare Figure 2
```{r fig2-panels, fig.height=8, fig.width=4.5, dependson= "calc-div"}
######################################################### OBSERVED RICHNESS ######################################################### 
...
## Plot Figure 2 
```{r fig2-noyaxis, fig.width = 7, fig.height = 7, dependson = "fig2-panels"}
...
## Prepare Figure S2
```{r figS2-panels, fig.height=8, fig.width=4.5}
######################################################### SHANNON ENTROPY ######################################################### 
...
prod_vs_simpseven_plot <-  
  ...
  # Add the R2 and p-value to the plot 
  annotate("text", x=0.14, y=15, label=lm_lab_perliter_simpseven_PA, parse = TRUE, color = "#FF6600", size = 4) +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank()) 
...
percell_prod_vs_simpseven_plot <- 
  ...
  scale_x_continuous(limits = c(0,0.15), breaks = seq(from = 0, to = 0.15, by = 0.03)) + 
  # Add the R2 and p-value to the plot 
  annotate("text", x=0.14, y=-6.3, label=lm_lab_percell_simpseven_PA, parse = TRUE, color = "#FF6600", size = 4) +
  theme(legend.title = element_blank(), legend.position ="none")
...
## Plot Figure S2
```{r figS2-noyaxis, fig.width = 8, fig.height = 8, dependson = c("figS2-panels", "calc-div")}
...
```{r simps-vs-prod, fig.width=4, fig.height=4, dependson= "calc-div"}
...
## Prepare Figure S7
```{r figS3, fig.width=12, fig.height=3.5, dependson= "calc-div"}
...
## Community Bulk Production: No correlation with PC2
...
## Per Capita Production --> Correlations for both Simpson and PC2
...
summary(lm(log10(fracprod_per_cell_noinf) ~ unweighted_div, data = lasso_data_df_particle)) 
```


# Perform Ridge and Lasso regression
## Bulk Community Production: PARTICLE 
```{r part-bulk-lasso-ridge, dependson = c("calc-div"), fig.width=8, fig.height=4}
...
The test MSE for ridge regression is `r mean((ridge_divs_pred - y_test)^2)` while the test MSE for lasso is `r mean((lasso_divs_pred - y_test)^2)`.  
...
```{r percapita-lasso-ridge, dependson = c("calc-div", "prepare-data"), fig.width=8, fig.height=4}
...
```{r ALL-bulk-lasso-ridge, dependson = c("calc-div"), fig.width=8, fig.height=4}
# Set the seed for reproducibility of the grid values 
set.seed(777)
...
```{r all-percapita-lasso-ridge, dependson = c("calc-div"), fig.width=8, fig.height=4}
set.seed(777)
...
```{r plot-unweighted-sesmpd, fig.width = 4.6, fig.height = 9.5, dependson = c("calc-div", "calc-sesmpd")}
...
```{r plot-weighted-sesmpd, fig.width = 4.5, fig.height = 9.5, dependson = c("calc-div", "calc-sesmpd")}
...
```{r cellnum-vs-sesmpd, fig.width=4.5, fig.height=4}
...
```{r plot-station-diversity, fig.width = 9, fig.height = 5, dependson = c("calc-div", "calc-sesmpd"), eval = FALSE}
# Test by station
...
# Figure SX: Compare Diversity Estimates
```{r plot-div, fig.width = 14, fig.height = 7, dependson= c("calc-div", "prepare-data")}
...
```{r, randomized-sesmpd, fig.width = 4.25, fig.height = 4, dependson = c("randomize-richness", "calc-div")}  
# Read in the tree
...
```{r temp-vs-prod, dependson = "prepare-data"}
ggplot(lasso_data_df, aes(x = Temp_C, y = frac_bacprod, color = fraction)) +
  ...
  xlab("Water Temperature (C)") + ylab("Bulk Community Production (ug C/L/day)") +
...
# Figure S1: Faiths PD
```{r faiths-pd, fig.width = 4.25, fig.height = 4}
surface_PAFL_otu_pruned_RAREFIED_rm2
