...
#fig_1 <- 
tiff(file="Cornell_prodcells.tiff", width = 8.5, height = 3.5, units = "in", res = 300)
...
plot_grid(prezzzz, season_legend ,
           ncol = 1, nrow = 2, 
...
prod_vs_rich_plot <-  
  ...
  geom_smooth(data=filter(richness, fraction == "Particle"), method='lm', color = "firebrick", fill = "firebrick") + 
  scale_x_continuous(limits = c(150,980), breaks = seq(from = 0, to =925, by = 150)) + 
   # Add the R2 and p-value to the plot 
  annotate("text", x=790, y=45, label=lm_lab_perliter_rich_PA, parse = TRUE, color = "firebrick", size = 4) +
...
percell_prod_vs_rich_plot <- 
  ...
  # Add the R2 and p-value to the plot 
  annotate("text", x=790, y=-7.5, label=lm_lab_percell_rich_PA, parse = TRUE, color = "firebrick", size = 4) +
...
prod_vs_invsimps_plot <-  
  ...
  geom_smooth(data=filter(invsimps, fraction == "Particle"), method='lm', color = "firebrick", fill = "firebrick") + 
  scale_x_continuous(limits = c(0,85), breaks = seq(from = 0, to = 85, by = 20)) + 
  # Add the R2 and p-value to the plot 
  annotate("text", x=70, y=45, label=lm_lab_perliter_invsimps_PA, parse = TRUE, color = "firebrick", size = 4) +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank()) 
...
percell_prod_vs_invsimps_plot <- 
  ...
  #scale_y_continuous(limits = c(-8e-7,5e-6), breaks = seq(from = 0, to = 6e-7, by = 3e-7)) + 
  # Add the R2 and p-value to the plot 
  annotate("text", x=70, y=-7.5, label=lm_lab_percell_invsimps_PA, parse = TRUE, color = "firebrick", size = 4) +
...
tiff(file="Cornell_BEFs.tiff", width = 7, height = 3, units = "in", res = 300)
plot_grid(p1, p2,
          ncol = 2, nrow = 1, 
          labels = c("A", "B"),
          rel_widths = c(1, 1))
