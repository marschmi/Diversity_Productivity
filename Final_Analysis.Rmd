...
## Figure 2
```{r fig2-panels, fig.width = 7, fig.height = 7, dependson= "calc-div"}
######################################################### OBSERVED RICHNESS ######################################################### 
...
```{r figs4-div-station-season, fig.width = 14, fig.height = 7, dependson= c("calc-div", "prepare-data")}
...
```{r figS5-panels, fig.width = 7, fig.height = 7, dependson = "calc-div"}
######################################################### SHANNON ENTROPY ######################################################### 
...
```{r figS7, fig.width=12, fig.height=3.5, dependson= "calc-div"}
...
## 1. Extract the R2 and p-value from the linear model: 
lm_lab_percell_free_mpd <- paste("atop(R^2 ==", round(summary(lm_percell_free_mpd)$adj.r.squared, digits = 2), ",",
             "p ==", round(unname(summary(lm_percell_free_mpd)$coefficients[,4][2]), digits = 3), ")")

## 2. Plot Weighted Pylogenetic Diersity vs Per Capita (Per-Cell) Production 
weight_percell_vs_mpd_plot <- 
  ...
  scale_y_continuous(limits = c(-8.5,-5), breaks = c(-8, -7, -6)) + 
  # Add the R2 and p-value to the plot 
  annotate("text", x=0.65, y=-7.9, label=lm_lab_percell_free_mpd, parse = TRUE, color = "deepskyblue3", size = 4) +
  theme(legend.title = element_blank(), legend.position ="none", 
        legend.text = element_text(size = 14))
...
```{r euclidian-pcoa-varexplained, fig.height=4, fig.width=5}
...
```{r multiple-regression-PCs, fig.width=3.5, fig.height=3.5}
