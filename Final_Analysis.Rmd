...
##### If you have any questions, please be welcome to email the corresponding author at marschmi@umich.edu
...
lm_divs_particle_comm <- div_measures_part %>%
  ...
  dplyr::select(fraction, dependent, measure, adj.r.squared, p.value) 
...
################ LASSO ################ 
# Run lasso regression with alpha = 1
lasso_divs_train_free_comm <- glmnet(x[train,], y[train], alpha = 1, lambda = grid, standardize = TRUE)
par(mfrow = c(1,2))
...
The lasso model for predicting the community-wide heterotrophic bacterial production for the free-living habitat uses *only* pH as the **best and only** predictor!
...
```{r all-comm-lasso, dependson = c("calc-div"), fig.width=8, fig.height=4}
# Set the seed for reproducibility of the grid values 
set.seed(777)      
...
```{r all-percapita-lasso, dependson = c("calc-div"), fig.width=8, fig.height=4}
set.seed(777) 
...
```{r figs4-div-station-season, fig.width = 14, fig.height = 7, dependson= c("calc-div", "prepare-data-lasso")}
...
# Table S1
```{r environ-lm, dependson = "euclidian-pcoa-varexplained"}
## Prepare the dataframes for Free and Particle data
metadata_pca_free <- filter(metadata_pca, fraction == "Free")
metadata_pca_part <- filter(metadata_pca, fraction == "Particle")
...
lm_variables <- c("frac_bacprod", "fracprod_per_cell_noinf", "Temp_C", "SpCond_uSpercm", "TDS_mgperL", 
                  ...
                  "DO_percent", "total_bac_abund","attached_bac", "dnaconcrep1", 
                  "PC1", "PC2", "PC3" ,"PC4", "PC5", "PC6") 

########################################################
##############  All Samples: Community-wide 
lm_summary_all_comm <- metadata_pca %>%
  ...
  mutate(fraction = "All Samples", dependent = "Community Production") %>%
  dplyr::select(fraction, dependent, independent, adj.r.squared, p.value) 

##############  All Samples: Per-capita  
lm_summary_all_percap <- metadata_pca %>%
...
########################################################
##############  Particle: Community-wide 
lm_summary_particle_comm <- metadata_pca_part %>%
  dplyr::select(one_of(lm_variables), -fracprod_per_cell_noinf) %>%
  gather(key = independent, value = measurement, -frac_bacprod) %>%
  mutate(measurement = as.numeric(measurement)) %>%
  group_by(independent) %>% 
...
##############  Particle: Per-capita  
lm_summary_particle_percap <- metadata_pca_part %>%
  dplyr::select(one_of(lm_variables), -frac_bacprod) %>%
  filter(!is.na(fracprod_per_cell_noinf)) %>%   # Remove samples that are not NA
  gather(key = independent, value = measurement, -fracprod_per_cell_noinf) %>%
  mutate(measurement = as.numeric(measurement)) %>%
...
########################################################
##############  Free: Community-wide 
lm_summary_free_comm <- metadata_pca_free %>%
  dplyr::select(one_of(lm_variables), -fracprod_per_cell_noinf) %>%
  gather(key = independent, value = measurement, -frac_bacprod) %>%
  mutate(measurement = as.numeric(measurement)) %>%
  group_by(independent) %>% 
...
##############  Free: Per-capita  
lm_summary_free_percap <- metadata_pca_free %>%
  dplyr::select(one_of(lm_variables), -frac_bacprod) %>%
  filter(!is.na(fracprod_per_cell_noinf)) %>%   # Remove samples that are not NA
  gather(key = independent, value = measurement, -fracprod_per_cell_noinf) %>%
  mutate(measurement = as.numeric(measurement)) %>%
...
individual_lm_results <- bind_rows(lm_div_results, all_pca_environ_lm_results) %>%
  mutate(adj.r.squared = round(adj.r.squared, digits = 2),
         p.value = round(p.value, digits = 4))
 
 
# Output the results for models that have a pvalue of less than 0.05
datatable(dplyr::filter(individual_lm_results, p.value < 0.05),
       caption = "Single Linear Model results from diversity, environmental, & PCA axes with a p-value less than 0.05.",
...
#### Per-Capita Production
```{r}
datatable(dplyr::filter(data.frame(individual_lm_results), dependent_var == "Per-Capita Production" & p.value < 0.05), 
       caption = "Significant (< 0.05) Linear Predictions for Per-Capita Heterotrophic Production axes with a p-value less than 0.05.",
       options = list(pageLength = 50))  
```

#### Community-Wide Production
```{r}
datatable(dplyr::filter(individual_lm_results, dependent_var == "Community Production" & p.value < 0.05),
       caption = "Significant (< 0.05) Linear Predictions for Community-Wide Heterotrophic Production axes with a p-value less than 0.05.",
       options = list(pageLength = 50)) 
```
...
## pH Analysis
```{r pH-analysis, fig.height=8, fig.width=8}
...
################ Free: Richness vs pH
## 1. Run the linear regression 
lm_pH_rich_free <- lm(mean ~ pH, data = dplyr::filter(richness, fraction == "Free"))

# 2. Extract the R2 and p-value from the linear model: 
lm_lab_pH_rich_free <- paste("atop(R^2 ==", round(summary(lm_pH_rich_free)$adj.r.squared, digits = 2), ",",
            "p ==", round(unname(summary(lm_pH_rich_free)$coefficients[,4][2]), digits = 2), ")")

################ Particle: Richness vs pH
## 3. Run the linear regression 
lm_pH_rich_part <- lm(mean ~ pH, data = dplyr::filter(richness, fraction == "Particle"))

# 4. Extract the R2 and p-value from the linear model: 
lm_lab_pH_rich_part <- paste("atop(R^2 ==", round(summary(lm_pH_rich_part)$adj.r.squared, digits = 2), ",",
            "p ==", round(unname(summary(lm_pH_rich_part)$coefficients[,4][2]), digits = 3), ")")

plot_rich_vs_pH <- 
  ggplot(data = richness, aes(x = pH, y = mean)) +
  geom_point(size = 3, aes(fill = fraction, shape = season)) + 
  ylab("Observed Richness") + 
  scale_shape_manual(values = season_shapes) + 
  scale_fill_manual(values = fraction_colors) + 
  #geom_smooth(method = "lm", color = "skyblue", fill = "skyblue", data = dplyr::filter(richness, fraction == "Free")) +
  geom_smooth(method = "lm", color = "#FF6600", fill = "#FF6600", data = dplyr::filter(richness, fraction == "Particle")) +
  # Add the R2 and p-value to the plot 
  annotate("text", y = 800, x=8.4, label=lm_lab_pH_rich_part, parse = TRUE, color = "#FF6600", size = 4) +
  annotate("text", y = 300, x=8.12, label=lm_lab_pH_rich_free, parse = TRUE, color = "deepskyblue3", size = 4) +
  theme(legend.title = element_blank(), legend.position = "none")  


################ Free: Inverse Simpson vs pH
## 1. Run the linear regression 
lm_pH_invsimps_free <- lm(mean ~ pH, data = dplyr::filter(invsimps, fraction == "Free"))

# 2. Extract the R2 and p-value from the linear model: 
lm_lab_pH_invsimps_free <- paste("atop(R^2 ==", round(summary(lm_pH_invsimps_free)$adj.r.squared, digits = 2), ",",
            "p ==", round(unname(summary(lm_pH_invsimps_free)$coefficients[,4][2]), digits = 2), ")")

################ Particle: Inverse Simpson vs pH
## 3. Run the linear regression 
lm_pH_invsimps_part <- lm(mean ~ pH, data = dplyr::filter(invsimps, fraction == "Particle"))

# 4. Extract the R2 and p-value from the linear model: 
lm_lab_pH_invsimps_part <- paste("atop(R^2 ==", round(summary(lm_pH_invsimps_part)$adj.r.squared, digits = 2), ",",
            "p ==", round(unname(summary(lm_pH_invsimps_part)$coefficients[,4][2]), digits = 4), ")")

plot_invsimps_vs_pH <-
  ggplot(data = invsimps, aes(x = pH, y = mean)) +
  geom_point(size = 3, aes(fill = fraction, shape = season)) + 
  ylab("Inverse Simpson") + 
  scale_shape_manual(values = season_shapes) + 
  scale_fill_manual(values = fraction_colors) + 
  #geom_smooth(method = "lm", color = "skyblue", fill = "skyblue", data = dplyr::filter(invsimps, fraction == "Free")) +
  geom_smooth(method = "lm", color = "#FF6600", fill = "#FF6600", data = dplyr::filter(invsimps, fraction == "Particle")) +
  # Add the R2 and p-value to the plot 
  annotate("text", y = 70, x=8.4, label=lm_lab_pH_invsimps_part, parse = TRUE, color = "#FF6600", size = 4) +
  annotate("text", y = 5, x=8.15, label=lm_lab_pH_invsimps_free, parse = TRUE, color = "deepskyblue3", size = 4) +
  theme(legend.title = element_blank(), legend.position = "none")  


pH_plots <- plot_grid(plot_pH_comm, plot_pH_per_cap, plot_rich_vs_pH, plot_invsimps_vs_pH, 
...
## Total Production 
```{r total-prod-vs-season-station, eval = TRUE, fig.width = 6, fig.height = 4, include = TRUE}
