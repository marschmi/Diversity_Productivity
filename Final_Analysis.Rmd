...
The code below represents the main analysis to reproduce all figures except Figure S6. To see how to reproduce figure S6 please see the github for this project.
...
datatable(dplyr::filter(lm_div_results, p.value < 0.05),
       caption = "Linear Model results from diversity metrics with a p-value less than 0.05.")
```
...
# Output the results for models that have a pvalue of less than 0.05
datatable(dplyr::filter(all_pca_environ_lm_results, p.value < 0.05),
       caption = "Single Linear Model results from environmental variables & PCA axes with a p-value less than 0.05.")
