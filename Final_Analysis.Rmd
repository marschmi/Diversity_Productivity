...
```{r fig2, fig.height=9, fig.width=5}
######################################################### OBSERVED RICHNESS 
...
rich_distribution_plot <- 
  ...
  geom_jitter(size = 3, shape = 21, aes(fill = fraction), width = 0.2) + 
  geom_boxplot(alpha = 0.5, outlier.shape = NA, aes(fill = fraction)) +
  scale_y_continuous(limits = c(0,950), breaks = seq(from = 0, to =925, by = 150)) + 
  ylab("Standardized Effect Size \n Unweighted Mean Pairwise Dist") +
  xlab("Fraction") + 
  ##### Particle vs free per-cell production 
  geom_path(data = rich_nums, aes(x = a, y = b), linetype = 1, color = "#424645") +
  annotate("text", x=1.5, y=850, fontface = "bold",  size = 4, color = "#424645",
...
prod_vs_rich_plot <-  
  ...
  scale_x_continuous(limits = c(0,950), breaks = seq(from = 0, to =925, by = 150)) +
  annotate("text", x = 750, y=3, color = "#FF6600", fontface = "bold",
...
percell_prod_vs_rich_plot <- 
  ...
  annotate("text", x = 750, y=-7.3, color = "#FF6600", fontface = "bold",
           label = paste("R2 =", round(summary(lm_prod_vs_rich)$adj.r.squared, digits = 2), "\n", 
                         "p =", round(unname(summary(lm_prod_vs_rich)$coefficients[,4][2]), digits = 3))) + 
...
plot_grid(rich_distribution_plot, prod_vs_rich_plot, percell_prod_vs_rich_plot,
          labels = c("A", "B", "C"), ncol = 1, nrow = 3,
          rel_heights = c(0.5, 0.8, 1.2),
          align = "v")
...
invsimps_distribution_plot <- 
  ...
  annotate("text", x=1.5, y=80, fontface = "bold",  size = 3.5, color = "#424645", label= "NS") +
  theme(legend.position = "none", #axis.title.y = element_blank(),
        axis.title.x = element_blank(), axis.text.x = element_blank()) +
  coord_flip()
...
```{r plot-sesmpd, fig.width = 5, fig.height = 11, dependson = c("calc-div", "calc-sesmpd")}
