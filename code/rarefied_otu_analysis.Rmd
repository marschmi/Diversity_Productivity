...
unweightedMPD_vs_fracprod_indepswap <- ggplot(filter(unweighted_sesMPD_indepswap, fraction %in% c("WholePart", "WholeFree")), 
  ...
  xlab("Unweighted Mean Pairwise Distance \n (ses.mpd)") + 
  ylab("Heterotrophic Production \n(μgC/L/hr)") +
  ggtitle("Unweighted MPD: Independent Swap") + 
  scale_color_manual(values = c("firebrick3", "cornflowerblue")) +
  geom_smooth(method = "lm") +
...
weightedMPD_vs_fracprod_indepswap <- ggplot(filter(WEIGHTED_sesMPD_indepswap, fraction %in% c("WholePart", "WholeFree")), 
  ...
  xlab("Weighted Mean Pairwise Distance  \n (ses.mpd)") + 
  ylab("Heterotrophic Production \n(μgC/L/hr)") +
  ggtitle("Weighted MPD: Independent Swap") + 
  scale_color_manual(values = c("firebrick3", "cornflowerblue")) +
  theme(legend.position = c(0.2, 0.87), legend.title = element_blank()) +
...
```{r indepswapdiv-vs-diversity, fig.width = 8, fig.height = 4, dependson = c("subset-diversity","load-data","vean-otu-alphadiv-calc", "load-indepswap")}
### Richness DF
...
summary(lm(mean ~ mpd.obs.z, data = combined_rich_unweightedMPD_indepswap))
...
divs_p1_indepswap <- ggplot(combined_rich_unweightedMPD_indepswap, aes(y = mean, x = mpd.obs.z)) + 
  ...
  annotate("text", x = -0.8, y=200, 
         color = "black", fontface = "bold",
         label = paste("R2 =", round(summary(lm(mean ~ mpd.obs.z, data = combined_rich_unweightedMPD_indepswap))$adj.r.squared, 
                                     digits = 2), "\n", 
                       "p =", round(unname(summary(lm(mean ~ mpd.obs.z, data = combined_rich_unweightedMPD_indepswap))$coefficients[,4][2]), 
                                    digits = 4))) 
