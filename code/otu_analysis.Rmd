...
comm_sesmpd_taxlab <- read.csv("../data/PD_rarefied_2895/comm_sesmpd_taxalab_weighted.csv", header = TRUE) %>%
  dplyr::rename(names = X) %>%
  make_metadata_norep()
# Set the factor levels to be the order we'd like
comm_sesmpd_taxlab$fraction <- factor(comm_sesmpd_taxlab$fraction,  levels = c("WholePart", "Particle", "WholeFree", "Free"))
comm_sesmpd_taxlab$lakesite <- factor(comm_sesmpd_taxlab$lakesite,  levels = c("MOT", "MDP", "MBR", "MIN"))
...
comm_sesmpd_taxlab_plot1 <- 
  ggplot(comm_sesmpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = season, fill = season)) + 
...
comm_sesmpd_taxlab_plot2 <- 
  ggplot(comm_sesmpd_taxlab, aes(x = fraction, y = mpd.obs.p, color = season, fill = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + 
  ylab("p-value") +
  geom_boxplot(alpha = 0.5) +
  scale_color_manual(values = season_colors) + 
...
ggplot(comm_sesmpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = mpd.obs.p, shape = lakesite)) + 
  geom_point(size = 3, position = position_jitterdodge()) + 
...
weighted_season_plot <- ggplot(comm_sesmpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = mpd.obs.p, shape = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + 
  ylab("Weighted Mean Pairwise Distance \n(ses.mpd)") + 
  geom_hline(yintercept = 0.05) +
  scale_colour_gradientn(colours = myPalette(100), limits=c(0.001, 1), name="p-value") +
