...
This analysis was inspired by <a href="http://kembellab.ca/r-workshop/biodivR/SK_Biodiversity_R.html" target="_blank">Steve Kembells's tutorial</a> on biodiversity analysis in R with the `Picante` package. 

```{r faiths_PD}
# Remove the sediment data 
#nosed_physeq <- subset_samples(otu_merged_musk_pruned, fraction != "Sediment" & limnion == "Top")
#nosed_physeq <- prune_taxa(taxa_sums(nosed_physeq) > 0, nosed_physeq)

# To calculate the sum of the phylogenetic branch length for samples, use the pd {picante} function
# First force the OTU 
#otu_musk <- matrix(otu_table(nosed_physeq), nrow = nrow(otu_table(nosed_physeq)))
#rownames(otu_musk) <- sample_names(nosed_physeq)
#colnames(otu_musk) <- taxa_names(nosed_physeq)
...
#relabund_otu <- decostand(otu_musk, method = "total")
# check total abundance in each sample
#apply(relabund_otu, 1, sum)

# Bring the tree out of the phyloseq object 
#tree_musk <- phy_tree(nosed_physeq)

# metadata 
#metadata <- data.frame(sample_data(nosed_physeq))

# check for mismatches/missing species between community data and phylo tree
#combined <- match.phylo.comm(tree_musk, relabund_otu)
...
#faiths_pd <- pd(relabund_otu, tree_musk, include.root = FALSE)
#faiths_pd$norep_filter_name <- row.names(faiths_pd)
  
#write.csv(faiths_pd, file = "../data/Faiths_PD.csv", row.names = FALSE)
faiths_pd <- read.csv("../data/FaithsPD_relprop/Faiths_PD.csv", header = TRUE)
```


## Faith's PD & Richness 
```{r faiths_PD_analysis}
# Join Faith's PD with the rest of the metadata 
nosed_meta_data_PD <- left_join(faiths_pd, nosed_meta_data, by = "norep_filter_name")


plot_season_PD <- ggplot(nosed_meta_data_PD, aes(x = season, y = PD, color = season, fill = season)) + 
  geom_jitter(size = 3) + geom_boxplot(alpha = 0.4) + ylab("Faith's PD") + 
  scale_fill_manual(values = season_colors) +
  scale_color_manual(values = season_colors) 

plot_lakesite_PD <- ggplot(nosed_meta_data_PD, aes(x = lakesite, y = PD, color = lakesite, fill = lakesite)) + 
  geom_jitter(size = 3) + geom_boxplot(alpha = 0.4) + ylab("Faith's PD") + 
  scale_fill_manual(values = lakesite_colors) +
  scale_color_manual(values = lakesite_colors) 

plot_grid(plot_season_PD, plot_lakesite_PD, 
          labels = c("A", "B"), 
          ncol = 2)

### Is there a correlation between species richness and faith's PD?
lm_PD_vs_SR <- lm(PD ~ SR, data = nosed_meta_data_PD)
ggplot(nosed_meta_data_PD, aes(y = PD, x = SR)) + 
  geom_point(size = 3) + xlab("Faith's PD") + 
  xlab("Species richness") +
  geom_smooth(method = "lm") +
    annotate("text", x = 3500, y=100, color = "black", fontface = "bold",
           label = paste("R2 =", round(summary(lm_PD_vs_SR)$adj.r.squared, digits = 3), "\n", 
                         "p =", round(unname(summary(lm_PD_vs_SR)$coefficients[,4][2]), digits = 52)))
```

Faith's PD is lower certain seasons and laksites.  However, Faith's PD is **highly correlated (!!!)** with species richness, and we already know that there are fewer species in summer and the Lake Michigan channel (MOT), so we need some way to compare phylogenetic diversity that takes this fact into account.



## MPD and MNTD

Notes from Steve Kembell's tutorial: 

- `mpd`: Calculates the mean pairwise distance between all species or individuals in each community.  
- `mntd`: Calculates the mean nearest taxon distance, which is the average distance separating each species or individual in the community from its closest heterospecific relative.  
- `ses.`: standardized effect size  
        + Standardized effect sizes describe the difference between average phylogenetic distances in the observed communities versus null communities generated with some randomization method, standardized by the standard deviation of phylogenetic distances in the null data:

The `mpd` and `mntd` functions differs slightly from the `pd` function in that they take a distance matrix as input rather than a phylogeny object. Therefore, we will use the `cophenetic` function to calculate the distance matrix from our phylogeny.  
...
#comm_sesmpd <- ses.mpd(comm, phy_dist, null.model = "richness", 
#                       abundance.weighted = FALSE, runs = 999)

#write.csv(comm_sesmpd, file = "../data/ses_mpd.csv", row.names = TRUE)

# calculate ses.mntd
#comm_sesmntd <- ses.mntd(comm, phy_dist, null.model = "richness", 
#                         abundance.weighted = FALSE, runs = 999)
```
...
- **Positive SES values (mpd.obs.z > 0) and high quantiles (mpd.obs.p > 0.95)** =  phylogenetic evenness relative to the null model. Higher MPD values indicate more phylogenetically different taxa within a community (@Ruiz-Gonzalez et al., 2015).   
- **Negative SES values and low quantiles (mpd.obs.p < 0.05)** = phylogenetic clustering relative to the null model. 



```{r ses_mpd_analysis}
comm_sesmpd <- read.csv(file = "../data/FaithsPD_relprop/ses_mpd.csv") %>%
  dplyr::rename(norep_filter_name = X)

comm_sesmpd <- inner_join(nosed_meta_data_PD, comm_sesmpd, by = "norep_filter_name")


# compare ses.mpd between fractions
plot_ses_mpd_seasonfrac <- 
  ggplot(comm_sesmpd, aes(x = fraction, y = mpd.obs.z, color = season, fill = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + ylab("Mean Phylogenetic Distance (SES)") + 
...
# compare ses.mpd between fractions
plot_ses_mpd_pval_seasonfrac <- 
  ggplot(comm_sesmpd, aes(x = fraction, y = mpd.obs.p, color = season, fill = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + ylab("p-value") +
  geom_boxplot(alpha = 0.5) +
  scale_color_manual(values = season_colors) + 
  scale_fill_manual(values = season_colors) + 
...
plot_grid(plot_ses_mpd_seasonfrac, plot_ses_mpd_pval_seasonfrac, 
          labels = c("A", "B"), 
          nrow = 2)
...
ggplot(comm_sesmpd, aes(x = fraction, y = mpd.obs.z, color = mpd.obs.p, shape = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + ylab("Mean Pairwise Distance (ses.mpd)") + 
  geom_hline(yintercept = 0.05) +
  scale_colour_gradientn(colours = myPalette(100), limits=c(0.001, 1), name="p-value") +
  #scale_colour_gradient2(low="red", high = "cornflowerblue", mid = "black", midpoint = 0.05) +
  facet_grid(.~fraction, scale = "free_x") 



ggplot(comm_sesmpd, aes(x = mpd.obs, y = mpd.rand.mean, color = fraction)) + 
  geom_point(size = 3) + 
  ylab("Mean mpd in Environmental Comminity") +
  xlab("Mean mpd in Null Comminity")
```


`MPD` is generally thought to be more sensitive to tree-wide patterns of phylogenetic clustering and eveness, while `MNTD` is more sensitive to patterns of evenness and clustering closer to the tips of the phylogeny.

# Rarefied analysis for phylogenetic tree 
```{r rarefied_phylotree2}
load("../data/otu_merged_musk_pruned.RData")
...
nosed_otu_merged_musk_RAREFIED <- rarefy_even_depth(nosed_otu_merged_musk_pruned, sample.size = min_lib,
                                                    verbose = FALSE, replace = TRUE)



#prune_taxa(taxa_sums(nosed_otu_merged_musk_RAREFIED) > 2, nosed_otu_merged_musk_RAREFIED) 

#prune_taxa(taxa_sums(nosed_otu_merged_musk_RAREFIED) > 100, nosed_otu_merged_musk_RAREFIED) 


meta_data_RAREFIED <- sample_data(nosed_otu_merged_musk_RAREFIED)
# To calculate the sum of the phylogenetic branch length for samples, use the pd {picante} function
# First force the OTU 
#otu_musk_RAREIFIED <- matrix(otu_table(nosed_otu_merged_musk_RAREFIED), nrow = nrow(otu_table(nosed_otu_merged_musk_RAREFIED)))
#rownames(otu_musk_RAREIFIED) <- sample_names(nosed_otu_merged_musk_RAREFIED)
#colnames(otu_musk_RAREIFIED) <- taxa_names(nosed_otu_merged_musk_RAREFIED)

# Convert the count data to relative abundance by dividing each value by sample
# total abundance with the vegan function `decostand` 
#relabund_otu_RAREFIED <- decostand(otu_musk_RAREIFIED, method = "total")
# check total abundance in each sample
#apply(relabund_otu_RAREFIED, 1, sum)

# Bring the tree out of the phyloseq object 
#tree_musk_RAREFIED <- phy_tree(nosed_otu_merged_musk_RAREFIED)

# metadata 
#metadata_RAREFIED <- data.frame(sample_data(nosed_otu_merged_musk_RAREFIED))

# check for mismatches/missing species between community data and phylo tree
#combined_RAREFIED <- match.phylo.comm(tree_musk_RAREFIED, relabund_otu_RAREFIED)
# the resulting object is a list with $phy and $comm elements.  replace our
# original data with the sorted/matched data
#phy_RAREFIED <- combined_RAREFIED$phy
#comm_RAREFIED <- combined_RAREFIED$comm

#write.nexus(phy_RAREFIED, file = "../data/PD_rarefied_2895/phy_RAREFIED.csv", translate = TRUE)
#write.csv(comm_RAREFIED, file = "../data/PD_rarefied_2895/comm_RAREFIED.csv", row.names = TRUE)
...
# Remove the tree from the phyloseq object to prevent heavy computation in the future
otu_merged_musk_pruned <- merge_phyloseq(tax_table(otu_merged_musk_pruned), sample_data(otu_merged_musk_pruned), otu_table(otu_merged_musk_pruned))
otu_merged_musk_pruned
```
...
```{r rarefied_phylotree2}
faiths_pd_RAREFIED <- read.csv("../data/PD_rarefied_2895/faithsPD_rarefy.csv", header = TRUE)

# Join Faith's PD with the rest of the metadata 
meta_dataPD_RAREFIED <- left_join(faiths_pd_RAREFIED, meta_data_RAREFIED, by = "norep_filter_name")


### Is there a correlation between species richness and faith's PD?
lm_PD_vs_SR_RARE <- lm(PD ~ SR, data = meta_dataPD_RAREFIED)
ggplot(meta_dataPD_RAREFIED, aes(y = PD, x = SR)) + 
  geom_point(size = 3) + xlab("Faith's PD") + 
  xlab("Species richness") +
  geom_smooth(method = "lm") +
    annotate("text", x = 600, y=45, color = "black", fontface = "bold",
           label = paste("R2 =", round(summary(lm_PD_vs_SR_RARE)$adj.r.squared, digits = 3), "\n", 
                         "p =", round(unname(summary(lm_PD_vs_SR_RARE)$coefficients[,4][2]), digits = 52)))


#phy_dist_RAREFIED <- cophenetic(phy_RAREFIED)

# calculate standardized effect size mean pairwise distance (ses.mpd)
#comm_sesmpd_taxalab <- ses.mpd(comm_RAREFIED, phy_dist_RAREFIED, null.model = "taxa.labels", 
#                               abundance.weighted = TRUE, runs = 999)

#write.csv(comm_sesmpd, file = "../data/PD_rarefied_2895/sesMPD_rarefy.csv", row.names = TRUE)
...
# CALCULATE THE MPD
#comm_sesmpd_taxalab_weighted <- ses.mpd(comm_RAREFIED, phy_dist_RAREFIED, null.model = "taxa.labels", 
#                              abundance.weighted = TRUE, runs = 999)

#write.csv(comm_sesmpd_taxalab_weighted, file = "/scratch/lsa_fluxm/marschmi/git_repos/Diversity_Productivity/data/PD_rarefied_2895/comm_sesmpd_taxalab_weighted.csv", 
#          row.names = TRUE)

WEIGHTED_mpd_taxlab <- read.csv("../data/PD_rarefied_2895/comm_sesmpd_taxalab_weighted.csv", header = TRUE) %>%
  dplyr::rename(norep_filter_name = X) 

WEIGHTED_mpd_taxlab <- left_join(WEIGHTED_mpd_taxlab, meta_data_RAREFIED, by = "norep_filter_name")


# Set the factor levels to be the order we'd like
WEIGHTED_mpd_taxlab$fraction <- factor(WEIGHTED_mpd_taxlab$fraction,  levels = c("WholePart", "Particle", "WholeFree", "Free"))
WEIGHTED_mpd_taxlab$lakesite <- factor(WEIGHTED_mpd_taxlab$lakesite,  levels = c("MOT", "MDP", "MBR", "MIN"))




# compare ses.mpd between fractions
comm_sesmpd_taxlab_plot1 <- 
  ggplot(WEIGHTED_mpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = season, fill = season)) + 
...
# compare ses.mpd between fractions
comm_sesmpd_taxlab_plot2 <- 
  ggplot(WEIGHTED_mpd_taxlab, aes(x = fraction, y = mpd.obs.p, color = season, fill = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + 
  ylab("p-value") +
  geom_boxplot(alpha = 0.5) +
  scale_color_manual(values = season_colors) + 
...
weighted_plots <- plot_grid(comm_sesmpd_taxlab_plot1, comm_sesmpd_taxlab_plot2, 
          labels = c("A", "B"), 
          nrow = 2)
...
ggplot(WEIGHTED_mpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = mpd.obs.p, shape = lakesite)) + 
  geom_point(size = 3, position = position_jitterdodge()) + 
...
weighted_season_plot <- ggplot(WEIGHTED_mpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = mpd.obs.p, shape = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + 
  ylab("Weighted Mean Pairwise Distance \n(ses.mpd)") + 
  geom_hline(yintercept = 0.05) +
  scale_colour_gradientn(colours = myPalette(100), limits=c(0.001, 1), name="p-value") +
...
#########
unweighted_sesmpd_taxlab <- read.csv("../data/PD_rarefied_2895/comm_sesmpd_taxalab_UNweight.csv", 
                                        header = TRUE) %>%
  dplyr::rename(norep_filter_name = X)

unweighted_mpd_taxlab <- left_join(unweighted_sesmpd_taxlab, meta_data_RAREFIED, by = "norep_filter_name")
# Set the factor levels to be the order we'd like
unweighted_mpd_taxlab$fraction <- factor(unweighted_mpd_taxlab$fraction,  levels = c("WholePart", "Particle", "WholeFree", "Free"))
unweighted_mpd_taxlab$lakesite <- factor(unweighted_mpd_taxlab$lakesite,  levels = c("MOT", "MDP", "MBR", "MIN"))


# compare ses.mpd between fractions
unweighted_sesmpd_taxlab_plot1 <- 
  ggplot(unweighted_mpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = season, fill = season)) + 
  ggtitle("Unweighted") +
  geom_point(size = 3, position = position_jitterdodge()) + 
  ylab("Mean Pairwise Distance") + 
  geom_boxplot(alpha = 0.5) +
...
# compare ses.mpd between fractions
unweighted_sesmpd_taxlab_plot2 <- 
  ggplot(unweighted_mpd_taxlab, aes(x = fraction, y = mpd.obs.p, color = season, fill = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) + 
  ylab("p-value") +
  geom_boxplot(alpha = 0.5) +
  scale_color_manual(values = season_colors) + 
...
unweighted_plots <- plot_grid(unweighted_sesmpd_taxlab_plot1, unweighted_sesmpd_taxlab_plot2, 
          labels = c("A", "B"), 
          nrow = 2)


unweighted_season_plot <- ggplot(unweighted_mpd_taxlab, aes(x = fraction, y = mpd.obs.z, color = mpd.obs.p, shape = season)) + 
  geom_point(size = 3, position = position_jitterdodge()) +
...
plot_grid(unweighted_plots, weighted_plots, 
          labels = c("1", "2"), 
          ncol = 2)

plot_grid(unweighted_season_plot + ggtitle("Unweighted"),
          weighted_season_plot+ ggtitle("Weighted"), 
          labels = c("1", "2"), 
          ncol = 2)
```
...
```{r mpd_vs_fracprod}
##########  UNWEIGHTED ANALYSIS 
lmFL_mpd_unweighted_vs_fracprod <- lm(frac_bacprod ~ mpd.obs.z, 
                                      data = filter(unweighted_mpd_taxlab, fraction == "WholeFree"))
summary(lmFL_mpd_unweighted_vs_fracprod)
lmPA_mpd_unweighted_vs_fracprod <- lm(frac_bacprod ~ mpd.obs.z, 
                                      data = filter(unweighted_mpd_taxlab, fraction == "WholePart" &
                                                      norep_filter_name != "MINEJ515"))
summary(lmPA_mpd_unweighted_vs_fracprod)

unweightedMPD_vs_fracprod <- ggplot(filter(unweighted_mpd_taxlab, fraction %in% c("WholePart", "WholeFree")), 
  ...
  geom_smooth(method = "lm", data = filter(unweighted_mpd_taxlab, fraction == "WholePart" &
                                                      norep_filter_name != "MINEJ515")) +
  theme(legend.position = c(0.2, 0.87)) +
    annotate("text", x = -3.5, y=20, 
           color = "firebrick3", fontface = "bold",
           label = paste("R2 =", round(summary(lmPA_mpd_unweighted_vs_fracprod)$adj.r.squared, digits = 4), "\n", 
                         "p =", round(unname(summary(lmPA_mpd_unweighted_vs_fracprod)$coefficients[,4][2]), digits = 4))) +
  annotate("text", x = 0, y=60, 
         color = "cornflowerblue", fontface = "bold",
         label = paste("R2 =", round(summary(lmFL_mpd_unweighted_vs_fracprod)$adj.r.squared, digits = 4), "\n", 
                       "p =", round(unname(summary(lmFL_mpd_unweighted_vs_fracprod)$coefficients[,4][2]), digits = 4))) 
...
lmFL_mpd_WEIGHTED_vs_fracprod <- lm(frac_bacprod ~ mpd.obs.z, 
                                      data = filter(WEIGHTED_mpd_taxlab, fraction == "WholeFree"))
summary(lmFL_mpd_WEIGHTED_vs_fracprod)
lmPA_mpd_WEIGHTED_vs_fracprod <- lm(frac_bacprod ~ mpd.obs.z, 
                                      data = filter(WEIGHTED_mpd_taxlab, fraction == "WholePart")) #&
                                                      #norep_filter_name != "MINEJ515"))
summary(lmPA_mpd_WEIGHTED_vs_fracprod)

weightedMPD_vs_fracprod <- ggplot(filter(WEIGHTED_mpd_taxlab, fraction %in% c("WholePart", "WholeFree")), 
       aes(x = mpd.obs.z, y = frac_bacprod, color = fraction)) + 
  geom_point(size = 3) +
  ...
  ggtitle("Weighted") + 
  scale_color_manual(values = c("firebrick3", "cornflowerblue")) +
  geom_smooth(method = "lm", data = filter(WEIGHTED_mpd_taxlab, fraction == "WholePart")) +
  theme(legend.position = c(0.2, 0.87)) +
    annotate("text", x = 0, y=20, 
           color = "firebrick3", fontface = "bold",
           label = paste("R2 =", round(summary(lmPA_mpd_WEIGHTED_vs_fracprod)$adj.r.squared, digits = 4), "\n", 
                         "p =", round(unname(summary(lmPA_mpd_WEIGHTED_vs_fracprod)$coefficients[,4][2]), digits = 4))) +
  annotate("text", x = -2, y=40, 
         color = "cornflowerblue", fontface = "bold",
         label = paste("R2 =", round(summary(lmFL_mpd_WEIGHTED_vs_fracprod)$adj.r.squared, digits = 4), "\n", 
                       "p =", round(unname(summary(lmFL_mpd_WEIGHTED_vs_fracprod)$coefficients[,4][2]), digits = 4))) 


plot_grid(unweightedMPD_vs_fracprod, weightedMPD_vs_fracprod,
          labels = c("A", "B"), 
          ncol = 2)
...
#########  SPEED TALK 
wholetops <- filter(nosed_meta_data, fraction %in% c("WholePart", "WholeFree") & limnion == "Top") %>%
  select(D0, fraction)

dff <- data.frame(matrix(ncol = 2, nrow = 12))
dff$X1 <- filter(wholetops, fraction == "WholePart")$D0
dff$X2 <- filter(wholetops, fraction == "WholeFree")$D0
dff <- rename(dff, Particle = X1, Free = X2)


wilcox.test(wholetops$D0)

obsrich_frac <- ggplot(wholetops, aes(x = fraction, y = D0)) +
  #geom_point(size = 3, position=position_jitterdodge(dodge.width=0.9)) +
  geom_boxplot(aes(fill = fraction, color = fraction), alpha = 0.5) +
  geom_jitter(aes( color = fraction), size = 3) +
...
#ggsave("../Figures/obsrich_frac.pdf", obsrich_frac, 
#       units = "in", height = 4, width = 5, dpi = 400)
...
lm_wholefreeonly_totprod_D0 <- lm(tot_bacprod ~ D0, data = wholefree_only)
# Plot the relationship between wholefree and total production 
speedtalk_totprod_wholefree_D0 <- ggplot(wholefree_only, aes(y = tot_bacprod, x = D0)) +
  geom_point(color = "cornflowerblue", size = 3) +
  geom_smooth(method = "lm", color = "cornflowerblue", se = FALSE, linetype = "longdash") + 
  annotate("text",  x = 800, y = 2, # For D2:  x = 40, y=5, 
...
lm_wholepartonly_totprod_D0 <- lm(tot_bacprod ~ D0, data = wholepart_only)
# Plot the relationship between wholepart and total production 
speedtalk_totprod_wholepart_D0 <- ggplot(wholepart_only, aes(y = tot_bacprod, x = D0)) +
  geom_point(color = "firebrick3",size = 3) +
  geom_smooth(method = "lm", color = "firebrick3") + 
  annotate("text",  x = 1400, y = 5, # For D2:  x = 40, y=5, 
           fontface = "bold", color = "firebrick3",
           label = paste("R2 =", round(summary(lm_wholepartonly_totprod_D0)$adj.r.squared, digits = 4), "\n", 
                         "p =", round(unname(summary(lm_wholepartonly_totprod_D0)$coefficients[,4][2]), digits = 4)))+
  theme(legend.position = c(0.12, 0.7), 
        legend.text = element_text(size = 10))
...
#plot_grid(speedtalk_totprod_wholefree_D0, speedtalk_totprod_wholepart_D0,
#          labels = c("A", "B"), 
#          ncol = 2)
...
ggsave("../Figures/fracprod_vs_D0.pdf", otu_pheno_D0, 
       units = "in", height = 4, width = 5, dpi = 400)

ggsave("../Figures/fracprod_vs_D2.pdf", otu_pheno_D2, 
       units = "in", height = 4, width = 5, dpi = 400)
```
...
# Can fractional production be predicted by phenoflow D0 observed richness? 
# FREE LIVING
free_otu_D0_percell <- lm(frac_bacprod/(total_bac_abund-attached_bac) ~ D0,
                        data = wholefree_2015_df)
summary(free_otu_D0_percell)

# PARTICLE
part_otu_D0_percell <- lm(frac_bacprod/attached_bac ~ D0, 
                          data = filter(wholeparticle_2015_df, attached_bac != 0))
summary(part_otu_D0_percell)

# Plot D0 Observed Richness
otu_pheno_D0_percell <- ggplot(all_top_2015_df, aes(x=D0, y=frac_bacprod, color = fraction)) + 
  geom_point(size = 3.5) + 
  geom_errorbarh(aes(xmin = D0 - D0_SD, xmax = D0 + D0_SD)) + 
  geom_errorbar(aes(ymin = frac_bacprod - SD_frac_bacprod, ymax = frac_bacprod + SD_frac_bacprod)) + 
  scale_color_manual(values = c("firebrick3","cornflowerblue"), limits = c("WholePart", "Free")) +
  ...
  theme(legend.position=c(0.15,0.9), legend.title=element_blank()) +
  annotate("text", x = 1000, y=35, color = "cornflowerblue", fontface = "bold",
           label = paste("R2 =", round(summary(free_otu_D0_percell)$adj.r.squared, digits = 4), "\n", 
                         "p =", round(unname(summary(free_otu_D0_percell)$coefficients[,4][2]), digits = 4))) + 
  annotate("text", x = 1600, y=8, color = "firebrick3", fontface = "bold",
           label = paste("R2 =", round(summary(part_otu_D0_percell)$adj.r.squared, digits = 4), "\n", 
                         "p =", round(unname(summary(part_otu_D0_percell)$coefficients[,4][2]), digits = 4)))
