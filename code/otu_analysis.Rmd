...
faiths_pd <- read.csv("../data/Faiths_PD.csv", header = TRUE)
```
...
```{r ses_mpd_analysis}
comm_sesmpd <- read.csv(file = "../data/ses_mpd.csv") %>%
  dplyr::rename(norep_filter_name = X)

comm_sesmpd <- inner_join(metadata, comm_sesmpd, by = "norep_filter_name")
