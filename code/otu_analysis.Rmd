...
```{r compare-diversities, fig.width=10, fig.height = 10}
...
ggplot(fraction_divs, aes(x = lakesite, y = div_value)) + 
...
```{r div_vs_seqdepth, fig.width=12, fig.height = 4}
...
lm_freeonly_totprod_D0_2014 <- lm(tot_bacprod ~ D0, data = filter(free_only, year == "2014"))
lm_freeonly_totprod_D0_2015 <- lm(tot_bacprod ~ D0, data = filter(free_only, year == "2015"))
...
ggplot(wholefree_only, aes(y = tot_bacprod, x = D0)) +
  geom_point(aes(color = lakesite), size = 3) +
  scale_color_manual(values = lakesite_colors) +
  geom_smooth(method = "lm", color = "black") + 
  ggtitle("WholeFree Only") + 
  annotate("text",  x = 750, y = 5, # For D2:  x = 40, y=5, 
           color = "black", fontface = "bold",
...
#summary(lm_partonly_totprod_D0)
# Plot the relationship 
...
lm_partonly_totprod_D0_2014 <- lm(tot_bacprod ~ D0, data = filter(part_only, year == "2014"))
lm_partonly_totprod_D0_2015 <- lm(tot_bacprod ~ D0, data = filter(part_only, year == "2015"))
# The trend is also insignificant for the whole particle fraction 
lm_wholepartonly_totprod_D0 <- lm(tot_bacprod ~ D0, data = wholepart_only)


ggplot(wholepart_only, aes(y = tot_bacprod, x = D0)) +
  geom_point(aes(color = lakesite), size = 3) +
  scale_color_manual(values = lakesite_colors) +
  geom_smooth(method = "lm", color = "black") + 
  ggtitle("WholePart Only") + 
  annotate("text",  x = 1200, y = 5, # For D2:  x = 40, y=5, 
           color = "black", fontface = "bold",
...
plot_D0_totprod <- plot_grid(plot_totprod_free_D0, plot_totprod_part_D0,
          labels = c("A", "B"), ncol = 2)
plot_D0_totprod


#ggsave(plot = plot_D0_totprod, 
#       filename = "../Figures/totbacprod_vs_D0.png", 
#       dpi = 600, 
#       height = 6, width = 16)
```
...
lm_freeonly_totprod_D1_2014 <- lm(tot_bacprod ~ D1, data = filter(free_only, year == "2014"))
lm_freeonly_totprod_D1_2015 <- lm(tot_bacprod ~ D1, data = filter(free_only, year == "2015"))
# The trend is also insignificant for the whole free fraction 
...
# Trend is NS in 2014 & 2015
lm_partonly_totprod_D1_2014 <- lm(tot_bacprod ~ D1, data = filter(part_only, year == "2014"))
lm_partonly_totprod_D1_2015 <- lm(tot_bacprod ~ D1, data = filter(part_only, year == "2015"))
# The trend is also insignificant for the whole particle fraction 
lm_wholepartonly_totprod_D1 <- lm(tot_bacprod ~ D1, data = wholepart_only)

plot_D1_totprod <- plot_grid(plot_totprod_free_D1, plot_totprod_part_D1,
          labels = c("A", "B"), ncol = 2)
plot_D1_totprod
...
plot_totprod_free_D2 <- ggplot(free_only, aes(y = tot_bacprod, x = D2)) +
  geom_point(aes(color = lakesite, shape = year), size = 3) +
  scale_color_manual(values = lakesite_colors) +
  xlim(0, 60) + # FOR D2
  #geom_smooth(method = "lm", color = "black") + 
  ggtitle("20 um Prefiltered Free-Living Only") + 
...
#summary(lm_partonly_totprod_D2)

# To put less weight in the outliers -> run robust regression
#rlm_part_totprod_D2 <- rlm(tot_bacprod ~ D2, data = part_only) # To perform robust regression 
...
plots_D2_particles_only <- plot_grid(title, 
                                     plot_grid(plot_totprod_partonly_2015_D2, plot_totprod_wholepartonly_D2, 
                                               labels=c('A', 'B')), 
