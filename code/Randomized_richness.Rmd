...
s <- data.frame(sample_data(notree_musk_surface_pruned_rm1))
...
# Create a new dataframe to hold the means and standard deviations of richness estimates
norep_filter_name <- row.names(otu_richness_rm5)
mean <- apply(otu_richness_rm5, 1, mean)
sd <- apply(otu_richness_rm5, 1, sd)
measure <- rep("Richness", nsamp)
otu_rich_stats_rm5 <- data.frame(norep_filter_name, mean, sd, measure)

# Create a new dataframe to hold the means and standard deviations of evenness estimates
norep_filter_name <- row.names(otu_evenness_rm5)
mean <- apply(otu_evenness_rm5, 1, mean)
sd <- apply(otu_evenness_rm5, 1, sd)
measure <- rep("Inverse_Simpson", nsamp)
otu_even_stats_rm5 <- data.frame(norep_filter_name, mean, sd, measure)

# Create a new dataframe to hold the means and standard deviations of shannon entropy estimates
norep_filter_name <- row.names(otu_shannon_rm5)
mean <- apply(otu_shannon_rm5, 1, mean)
sd <- apply(otu_shannon_rm5, 1, sd)
measure <- rep("Shannon_Entropy", nsamp)
otu_shan_stats_rm5 <- data.frame(norep_filter_name, mean, sd, measure)
...
sd <- apply(otu_evenness_rm5, 1, sd)
measure <- rep("Inverse_Simpson", nsamp)
otu_simpseven_stats_rm5 <- data.frame(norep_filter_name, mean, sd, measure)

# Calculate Simpson's Evenness into new df called "simps_evenness"
otu_simps_evenness_rm5 <- inner_join(otu_rich_stats_rm5, otu_even_stats_rm5, by = "norep_filter_name") %>%
  mutate(mean = mean.y/mean.x,
         sd = sd(mean),
         measure = "Simpsons_Evenness") %>%
  dplyr::select(norep_filter_name, mean, sd, measure)
...
# Combine alpha diversity into one dataframe 
otu_alpha_rm5 <- rbind(otu_rich_stats_rm5, otu_even_stats_rm5, otu_simps_evenness_rm5, otu_shan_stats_rm5)
otu_alphadiv_rm5 <- merge(otu_alpha, s, by = "norep_filter_name") %>%
  mutate(fraction = factor(fraction, levels = c("WholePart", "Particle", "WholeFree", "Free")),
         lakesite = factor(lakesite,  levels = c("MOT", "MDP", "MBR", "MIN")),
         measure = factor(measure, levels = c("Richness", "Simpsons_Evenness", "Shannon_Entropy", "Inverse_Simpson"))) 
...
# Create a new dataframe to hold the means and standard deviations of richness estimates
norep_filter_name <- row.names(otu_richness_rm10)
mean <- apply(otu_richness_rm10, 1, mean)
sd <- apply(otu_richness_rm10, 1, sd)
measure <- rep("Richness", nsamp)
otu_rich_stats_rm10 <- data.frame(norep_filter_name, mean, sd, measure)

# Create a new dataframe to hold the means and standard deviations of evenness estimates
norep_filter_name <- row.names(otu_evenness_rm10)
mean <- apply(otu_evenness_rm10, 1, mean)
sd <- apply(otu_evenness_rm10, 1, sd)
measure <- rep("Inverse_Simpson", nsamp)
otu_even_stats_rm10 <- data.frame(norep_filter_name, mean, sd, measure)

# Create a new dataframe to hold the means and standard deviations of shannon entropy estimates
norep_filter_name <- row.names(otu_shannon_rm10)
mean <- apply(otu_shannon_rm10, 1, mean)
sd <- apply(otu_shannon_rm10, 1, sd)
measure <- rep("Shannon_Entropy", nsamp)
otu_shan_stats_rm10 <- data.frame(norep_filter_name, mean, sd, measure)
...
sd <- apply(otu_evenness_rm10, 1, sd)
measure <- rep("Inverse_Simpson", nsamp)
otu_simpseven_stats_rm10 <- data.frame(norep_filter_name, mean, sd, measure)

# Calculate Simpson's Evenness into new df called "simps_evenness"
otu_simps_evenness_rm10 <- inner_join(otu_rich_stats_rm10, otu_even_stats_rm10, by = "norep_filter_name") %>%
  mutate(mean = mean.y/mean.x,
         sd = sd(mean),
         measure = "Simpsons_Evenness") %>%
  dplyr::select(norep_filter_name, mean, sd, measure)
...
# Combine alpha diversity into one dataframe 
otu_alpha_rm10 <- rbind(otu_rich_stats_rm10, otu_even_stats_rm10, otu_simps_evenness_rm10, otu_shan_stats_rm10)
otu_alphadiv_rm10 <- merge(otu_alpha, s, by = "norep_filter_name") %>%
  mutate(fraction = factor(fraction, levels = c("WholePart", "Particle", "WholeFree", "Free")),
         lakesite = factor(lakesite,  levels = c("MOT", "MDP", "MBR", "MIN")),
         measure = factor(measure, levels = c("Richness", "Simpsons_Evenness", "Shannon_Entropy", "Inverse_Simpson"))) 
```
...
all_divs <- bind_rows(rich_rm1, rich_rm5, rich_rm10)


# Remove singletons
rich_rm1 <- dplyr::filter(otu_alphadiv, measure == "Richness" & fraction %in% c("WholePart", "WholeFree") & year == "2015") %>%
  mutate(Removed = "1-tons")
# Remove 5-tons
rich_rm5 <- dplyr::filter(otu_alphadiv_rm5, measure == "Richness" & fraction %in% c("WholePart", "WholeFree") & year == "2015") %>%
  mutate(Removed = "5-tons")
# Remove 10-tons
rich_rm10 <- dplyr::filter(otu_alphadiv_rm10, measure == "Richness" & fraction %in% c("WholePart", "WholeFree") & year == "2015") %>%
  mutate(Removed = "10-tons")



### PLOT
ggplot(all_richness, aes(y = mean, x = Removed, color = Removed, fill = Removed)) +
