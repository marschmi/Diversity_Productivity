...
lm_phylo_rich <- summary(lm(hillR_phylo ~ Observed, data = filter(comb_div_df, Diversity == "Species richness")))
lm_phylo_shan <- summary(lm(hillR_phylo ~ Observed, data = filter(comb_div_df, Diversity == "Shannon diversity")))
lm_phylo_simps <- summary(lm(hillR_phylo ~ Observed, data = filter(comb_div_df, Diversity == "Simpson diversity")))
...
######################################################### OBSERVED RICHNESS ######################################################### 
rich_fraction_wilcox <- wilcox.test(Observed ~ fraction, data = dplyr::filter(div_iNEXT, Diversity == "Species richness"))
rich_fraction_wilcox

div_iNEXT %>%
  dplyr::filter(Diversity == "Species richness") %>%
  group_by(fraction) %>%
  summarize(mean(Observed), sd(Observed), min(Observed), max(Observed))
...
rich_distribution_plot <- 
  div_iNEXT %>%
  dplyr::filter(Diversity == "Species richness") %>%
  ggplot(aes(y = Observed, x = fraction)) +
  scale_fill_manual(values = fraction_colors) +
  geom_jitter(size = 3, aes(fill = fraction, shape = season), width = 0.2) + 
  geom_boxplot(alpha = 0.5, outlier.shape = NA, aes(fill = fraction)) +
