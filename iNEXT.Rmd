...
# And plot it
ggplot(comb_div_df, aes(x = Observed, y = hillR_phylo)) + 
  geom_point() + geom_smooth(method = "lm") +
  facet_wrap(~Diversity, scales = "free")
# Test it
summary(lm(hill_phylo~Observed, data = filter(comb_div_df, Diversity == "Species richness")))
summary(lm(hill_phylo~Observed, data = filter(comb_div_df, Diversity == "Shannon diversity")))
summary(lm(hill_phylo~Observed, data = filter(comb_div_df, Diversity == "Simpson diversity")))

# Check out Simpsons more 
comb_div_df %>%
  filter(Diversity == "Simpson diversity") %>%
  ggplot(aes(x = fraction, y = hillR_phylo, shape = season, fill = fraction)) + 
  geom_jitter(size = 3, width = 0.2) + 
  scale_fill_manual(values = fraction_colors,) +
  scale_shape_manual(values = season_shapes) 
