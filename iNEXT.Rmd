...
```{r corr-hill-phylo, fig.width=8, fig.height = 3.5}
...
## Mean Phylogenetic Pairwise Distance (ses.MPD)
...
#### Community Production: Particle 
```{r comm-part-lasso,fig.width=8, fig.height=4, warning = TRUE}
...
```{r comm-free-lasso, fig.width=8, fig.height=4, warning = TRUE}
# Set the seed for reproducibility of the grid values 
set.seed(777) 
...
## Figure 1
```{r Figure-1, eval = TRUE, fig.width = 8.5, fig.height = 3.5, warning = FALSE}
######################################################### Fraction ABUNDANCe 
...
```{r lm-model-objects, fig.width = 8.5, fig.height = 3.5}
...
hill_labels <- c(
  `expression({}^0*italic(D))` = "richness",
  `expression({}^1*italic(D))` = "shannon",
  `expression({}^2*italic(D))` = "simpson")
...
#timeseries_diff_plot1 <-
  ...
  tidyr::spread(key = fraction, value = div_val) %>%
  mutate(frac_difference = Particle-Free,
         hill_labels = factor(Diversity, labels = c("{}^0*italic(D)", "{}^1*italic(D)", "{}^2*italic(D)"))) %>%
  ...
  geom_point(data = dplyr::filter(mean_div_df, Diversity %in% c("richness", "shannon", "simpson")),
             aes(x = season, y = mean_diff), shape = 95, size = 20) +
  geom_line(data = dplyr::filter(mean_div_df, Diversity %in% c("richness", "shannon", "simpson")), 
            aes(x = season, y = mean_diff, group = Diversity), size = 1.5, color = "black", alpha = 0.5) + 
  facet_grid(hill_labels~., scales = "free", labeller = label_parsed) + 
  labs(y = "Difference in Diversity (Particle - Free)", 
       title = expression({}^q*italic(D))) +
...
timeseries_diff_plot2 <- 
  ...
  geom_hline(yintercept = 0, color = "grey") + 
  geom_jitter(aes(shape = lakesite), fill = "grey", size = 3, width = 0.1) + 
  geom_point(data = dplyr::filter(mean_div_df, Diversity %in% c("phylo_richness", "phylo_shannon", "phylo_simpson")),
             aes(x = season, y = mean_diff), shape = 95, size = 20) +
  geom_line(data = dplyr::filter(mean_div_df, Diversity %in% c("phylo_richness", "phylo_shannon", "phylo_simpson")), 
            aes(x = season, y = mean_diff, group = Diversity), size = 1.5, color = "black", alpha = 0.5) + 
  facet_grid(Diversity~., scales = "free") + 
  labs(y = "Difference in Diversity (Particle - Free)", 
       title = expression(textstyle("Phylogenetic")~{}^q*italic(D))) +
  scale_color_manual(values = c("#5B1A18",  "#FD6467", "#F1BB7B")) + #"#D67236",
  scale_shape_manual(values = lakesite_shapes) +
  theme(axis.title.x = element_blank())
...
# Figure 2
```{r Figure2, fig.width = 10, fig.height = 7}
# All plots together 
...
phylorich_distribution_plot <- 
  ...
  labs(y = expression(textstyle("Phylogenetic")~{}^0*italic(D)),
       x = "Fraction") +
  scale_shape_manual(values = season_shapes) +
...
prod_vs_phylorich_plot <-  
  ...
  scale_fill_manual(values = fraction_colors) +
  scale_shape_manual(values = season_shapes) +
  labs(x = expression(textstyle("Phylogenetic")~{}^0*italic(D)),
       y = "\n Community Production \n (μgC/L/day)") +
  geom_smooth(data=filter(wide_div_meta_df, fraction == "Particle"), 
              method='lm', color = "#FF6600", fill = "#FF6600") + 
...
phylorich_plots <- plot_grid(phylorich_distribution_plot, prod_vs_phylorich_plot, percell_prod_vs_phylorich_plot,
          #labels = c("B", "D", "F"), 
          ncol = 1, nrow = 3,
          rel_heights = c(0.5, 0.8, 1.1),
          align = "v")
...
phyloshan_distribution_plot <- 
  ...
  scale_y_continuous(limits = phyloshan_limits, breaks = phyloshan_breaks, expand = c(0,0)) + 
   labs(y = expression(textstyle("Phylogenetic")~{}^1*italic(D)),
       x = "Fraction") +
  scale_shape_manual(values = season_shapes) +
    # Add line and pval
...
prod_vs_phyloshan_plot <-  
  ...
  scale_shape_manual(values = season_shapes) +
  scale_fill_manual(values = fraction_colors) +
  labs(x = expression(textstyle("Phylogenetic")~{}^1*italic(D)),
       y = "\n Community Production \n (μgC/L/day)") +
...
phylosimps_distribution_plot <- 
  ...
  labs(y = expression(textstyle("Phylogenetic")~{}^2*italic(D)),
       x = "Fraction") +
...
prod_vs_phylosimps_plot <-  
  ...
  scale_fill_manual(values = fraction_colors) +
  scale_shape_manual(values = season_shapes) +
  labs(x = expression(textstyle("Phylogenetic")~{}^2*italic(D)),
       y = "Community Production \n (μgC/L/day)") +
...
# Plot Inverse simpson plots altogether
phylosimps_plots <- plot_grid(phylosimps_distribution_plot, prod_vs_phylosimps_plot, percell_prod_vs_phylosimps_plot,
          labels = c("B", "D", "F"), ncol = 1, nrow = 3,
          rel_heights = c(0.5, 0.8, 1.1),
          align = "v")
```
...
# Plot Phylodiv
```{r plot-BEFs-phylodiv, fig.width = 10, fig.height = 7}
# All plots together 
phylorich_plots <- plot_grid(phylorich_distribution_plot, prod_vs_phylorich_plot, 
...
phyloshan_plots_noyaxis <- plot_grid(phyloshan_distribution_plot + 
                                     theme(axis.title.y = element_blank(), axis.text.y = element_blank(),
                                           #  specify top, right, bottom, and left margins
                                           plot.margin = margin(0, 0, 0, 0.6, "cm")), 
          prod_vs_phyloshan_plot + theme(axis.title.y = element_blank(), axis.text.y = element_blank()), 
          percell_prod_vs_phyloshan_plot + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), legend.position = "none"),
          labels = c("B", "E", "H"), 
          ncol = 1, nrow = 3,
          rel_heights = c(0.5, 0.8, 1),
          align = "v")

phylosimps_plots_noyaxis <- 
  plot_grid(phylosimps_distribution_plot + 
                                     theme(axis.title.y = element_blank(), axis.text.y = element_blank(),
                                           #  specify top, right, bottom, and left margins
                                           plot.margin = margin(0, 0, 0, 0.6, "cm")), 
            prod_vs_phylosimps_plot + theme(axis.title.y = element_blank(),  axis.text.y = element_blank()), 
            percell_prod_vs_phylosimps_plot + theme(axis.title.y = element_blank(), axis.text.y = element_blank(), legend.position = "none"), 
          labels = c("C", "F", "I"), ncol = 1, nrow = 3,
          rel_heights = c(0.5, 0.8, 1),
          align = "v")

# Extract distribution plots of richness and inverse simpson
BEFs_phylodiv <- plot_grid(phylorich_plots, phyloshan_plots_noyaxis, phylosimps_plots_noyaxis,
          ncol = 3, nrow = 1, rel_widths = c(1, 0.75, 0.75),
          align = "h")


######## PLOT FIGURE 2
plot_grid(BEFs_phylodiv, season_legend,
                   ncol = 1, nrow = 2, 
                   rel_heights = c(1, 0.05))
```
...
# Phylodiv vs ses.MPD
```{r phylodiv-sesMPD, fig.width = 3.5, fig.height=3.5}
...
```{r singleton-iNEXT, eval = FALSE, echo = FALSE}
# THIS SECTION IS FOR Sanity Checks
...
```{r singleton-hillR, eval = FALSE, echo = FALSE}
...
```{r compare-coors, eval = FALSE, echo = FALSE}
...
```{r singleton-richness-plot, fig.width= 6, fig.height= 3.5, eval = FALSE, echo = FALSE}
...
```{r hillR-vs-prod-singleton, eval = FALSE, echo = FALSE}
### PD and Productivity
...
```{r BEF-comparison, eval = FALSE, echo = FALSE}
### Compare with doubleton analysis
...
```{r hilldiv, eval = FALSE, echo = FALSE}
