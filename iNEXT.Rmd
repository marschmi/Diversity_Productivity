...
```{r plot-iNEXT, fig.width = 10, fig.height = 6}
# Sample‐size‐based R/E curves, separating by "site""
ggiNEXT(iNEXT_data, type=1, facet.var="order") + facet_wrap(~order, scales="free") +
  scale_shape_manual(values = dat_iNEXT$shape) +
  scale_color_manual(values = dat_iNEXT$fraction_color) +
  scale_fill_manual(values = dat_iNEXT$fraction_color) + 
  geom_vline(xintercept = 6664, linetype="solid", color = "black", size=0.5) 
```


```{r plot-BEF, fig.width = 8.5, fig.height = 3.5}
# linear model for species richness
...
# linear model for inverse simpson 
...
ggplot(div_iNEXT, aes(x=Observed, y=frac_bacprod)) + 
  # X-axis errorbars
