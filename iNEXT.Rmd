...
## Calculate Phylo Hill Diversity
...
```{r corr-hill-phylo, fig.width=8, fig.height = 3}
...
## Figure 2
```{r Figure2, fig.width = 6.5, fig.height = 7}
# All plots together 
...
invsimps_plots_noyaxis <- 
  plot_grid(invsimps_distribution_plot + 
                                     theme(axis.title.y = element_blank(), axis.text.y = element_blank(),
                                           ...
                                           plot.margin = margin(0.2, 0, 0.2, 0.6, "cm")), 
...
```{r timeseries-plot, fig.width=3.5, fig.height=8}
...
plot_grid(ts_p1, ts_legend, ncol = 1, nrow= 2,
          rel_heights = c(1, 0.05))
...
```{r timeseries-means, fig.width=6, fig.height=7}
## PLOT THE MEANS
wide_div_meta_df %>%
...
# Plot hill divs by q
```{r hill-div-plots, fig.width=3.5, fig.height=3.5}
...
plot_grid(hill_nums_p, season_legend, nrow = 2, ncol = 1, rel_heights = c(1, 0.05))
```
...
## Figure S2
```{r figure_S2, fig.width = 10, fig.height = 4.5}
# h = 0 = richness
# h = 1 = exp(Shannon)
...
```{r iNEXT-richness-plot, fig.width= 6, fig.height= 3.5}
...
```{r Figure_S3, fig.width = 12, fig.height = 4, dependson="load-data"}
work_df <- metadata %>%
...
## Figure S4
```{r Figure_S4, fig.width = 4, fig.height = 7.2}
shannon_plots_noyaxis <- plot_grid(shannon_distribution_plot + 
                                     theme(#  specify top, right, bottom, and left margins
                                           plot.margin = margin(0.2, 0, 0, 0.6, "cm")), 
          prod_vs_shannon_plot, 
...
####### Legend for season
leg_plot <- ggplot(metadata, aes(y = frac_bacprod, x = fraction, fill = fraction, shape = season)) + 
  geom_jitter(size = 3, width = 0.2) + 
  scale_fill_manual(values = fraction_colors, guide = guide_legend(override.aes = list(shape = 22, size = 4))) +
  scale_shape_manual(values = season_shapes, guide = guide_legend(override.aes = list(size = 4))) +
  theme(legend.position = "bottom",  legend.justification="center",
        ...
        plot.margin = unit(c(0.2,0.2,1,0.2), "cm")) # top, right, bottom, and left margins
# Extract the legend
s_leg <- cowplot::get_legend(leg_plot + theme(legend.direction = "horizontal",legend.justification="center" ,legend.box.just = "bottom"))

######## PLOT FIGURE S4
plot_grid(shannon_plots_noyaxis, s_leg,
                   ncol = 1, nrow = 2, 
                   rel_heights = c(1, 0.08))
```
...
tableS1 <- individual_lm_results %>%
  ...
  arrange(-logLik)
#write.csv(tableS1, file = "tables/Table_S1.csv", quote = FALSE, row.names = FALSE) 
...
tableS2 <- individual_lm_results %>%  
  ...
  dplyr::select(-c(dependent_var, p.value))  %>%
  arrange(-logLik)
#write.csv(tableS2, file = "tables/Table_S2.csv", quote = FALSE, row.names = FALSE) 
